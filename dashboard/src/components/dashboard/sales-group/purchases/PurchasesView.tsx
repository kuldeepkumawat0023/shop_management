...
import { Plus, Download, ShoppingCart, Truck, Wallet, FileText, ChevronRight } from 'lucide-react';
...
export default function PurchasesView() {
  ...
  const columns = [
    ...
    { header: t('purchases.purchasesView.actions'), accessorKey: 'actions', cell: (row: any) => (
      <ActionButtons
        module="purchases"
        view={{ href: `/purchases/${row._id}` }}
        print={{ href: `/purchases/${row._id}` }}
        edit={{ href: `/purchases/${row._id}/edit` }}
        delete={{ onClick: () => handleDeleteClick(row._id, row.id) }}
      />
    )},
  ];
