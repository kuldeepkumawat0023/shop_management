        ...
        <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <div className="flex items-center gap-2">
                <h2 className="text-2xl font-black text-on-surface tracking-tight">{t('finance.paymentDetailView.paymentVoucher')}</h2>
          ...
          <div className="flex items-center gap-3 w-full sm:w-auto">
            <Button variant="outline" className="flex-1 sm:flex-none border-outline-variant/30 text-on-surface-variant hover:text-primary hover:bg-primary/10 font-semibold gap-2 rounded-xl transition-colors">
              ...
              <span className="hidden sm:inline">{t('finance.paymentDetailView.share')}</span>
            </Button>
            <Button className="flex-1 sm:flex-none gradient-button text-white border-none shadow-md shadow-primary/20 font-semibold gap-2 rounded-xl">
              ...
              <span className="hidden sm:inline">{t('finance.paymentDetailView.downloadPdf')}</span>
            </Button>
            <Button variant="outline" className="flex-1 sm:flex-none border-outline-variant/30 text-on-surface-variant hover:text-primary hover:bg-primary/10 font-semibold gap-2 rounded-xl transition-colors">
              <Printer className="w-4 h-4" />
              <span className="hidden sm:inline">{t('finance.paymentDetailView.print')}</span>
            </Button>
          </div>
        </div>
      </div>
      ...
      <div className="p-4 md:p-6 lg:p-8 w-full flex flex-col gap-6">
        ...
        <div className="bg-surface-container-lowest border border-outline-variant/30 rounded-3xl shadow-sm overflow-hidden flex flex-col relative print:shadow-none print:border-none">
          ...
          <div className="bg-success/10 border-b border-success/20 p-6 flex flex-col items-center justify-center text-center relative overflow-hidden">
            ...
            <h3 className="text-2xl font-black text-success uppercase tracking-wider relative z-10">{t('finance.paymentDetailView.paymentReceived')}</h3>
            <p className="text-sm font-bold text-success/80 mt-1 relative z-10">{t('finance.paymentDetailView.transactionSuccessful')}</p>
          </div>
          ...
          <div className="p-6 md:p-10 flex flex-col gap-8">
            ...
            <div className="flex flex-col items-center justify-center text-center border-b border-outline-variant/20 pb-8">
              <span className="text-sm font-bold text-on-surface-variant uppercase tracking-widest mb-2">{t('finance.paymentDetailView.amount')}</span>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-8 border-b border-outline-variant/20 pb-8">
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-2 text-primary font-bold mb-1">
                  ...
                  {t('finance.paymentDetailView.partyDetails')}
                </div>
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">{t('finance.paymentDetailView.receivedFrom')}</span>
              ...
              <div className="flex flex-col gap-4">
                <div className="flex items-center gap-2 text-primary font-bold mb-1">
                  ...
                  {t('finance.paymentDetailView.paymentInfo')}
                </div>
                <div className="flex flex-col">
                  <span className="text-xs font-bold text-on-surface-variant uppercase tracking-wider">{t('finance.paymentDetailView.method')}</span>
            ...
            <div className="flex flex-col gap-3">
              <div className="flex items-center gap-2 text-primary font-bold mb-1">
                ...
                {t('finance.paymentDetailView.notesReference')}
              </div>
